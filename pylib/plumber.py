...
def left_eye_pipeline() -> Gst.Pad:
    ...
    h=Homography(1280, 720, pitch_angle_degrees=15, clockwise=False)
    h.translate()
    perspective_correct = g.GlShaderHomography(name="perspective_left", matrix=h.matrix.T.flatten().tolist())
...
def right_eye_pipeline() -> Gst.Pad:
    ...
    # Add perspective correction before rotation (same for both cameras)
    h=Homography(1280, 720, pitch_angle_degrees=15, clockwise=True)
    h.translate()
